...
object InputParser {
  ...
  def stringToLandSize(str: String): (Int, Int) = {
    str.split(" ").filter(s => s.nonEmpty) match {
      case Array(width, height) =>
        (
          Try(width.toInt).getOrElse(
            throw DonneesIncorectesException("Width must be a number")
          ),
          Try(height.toInt).getOrElse(
            throw DonneesIncorectesException("Height must be a number")
          )
        )
      case _ =>
        throw DonneesIncorectesException(
          "First line of the file must land width and height sperated by a space"
        )
    }
  }
  ...
  def stringToMower(str: String): Mower = {
    str.split(" ").filter(s => s.nonEmpty) match {
      case Array(x, y, orientation) =>
        Mower(
          Position(
            Try(x.toInt).getOrElse(
              throw DonneesIncorectesException("X position must be a number")
            ),
            Try(y.toInt).getOrElse(
              throw DonneesIncorectesException("Y position must be a number")
            )
          ),
      ...
      case _ =>
        throw DonneesIncorectesException(
          "Line must be mower position and orientation sperated by a space"
        )
    }
  }
  ...
  def parseCliArgument(cliArgs: Map[CliArgument, CliArgumentValue]): Input = {
    ...
    inputType match {
      case List(CliArgumentValueInput(src), CliArgumentValueInputTypeFile()) =>
        TextFileInput(src)
