...
object YamlSerializable {
  ...
  implicit val serializeMowerHandler: Serializable[List[MowerHandler]] = (truc: List[MowerHandler], depth: Int) => {
    truc match {
      case Nil => ""
      case MowerHandler(mower, instructions, finalMower) :: tail =>
        s"""|- debut:
            ...
            |${getSpace(depth + 1)}direction: ${Orientation.toChar(mower.getOrientation).toString}
            ...
            |${getSpace(depth + 1)}direction: ${Orientation.toChar(finalMower.getOrientation).toString}
            |${serializeMowerHandler.serialize(tail, depth)}""".stripMargin
    }
  }

  implicit val serializeInstructions: Serializable[List[Instruction]] = (instructions: List[Instruction], depth: Int) => {
    instructions match {
      case Nil => ""
...
case class YamlFileOutput(name: String) extends FileOutput(name) {
  def writeOutput(configuration: LandResult) = {
    ...
    file.overwrite(YamlSerializable.serializeLandResult.serialize(configuration, 1))
    ()
  }
}
