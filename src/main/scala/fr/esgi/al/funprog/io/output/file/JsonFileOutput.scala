...
object JsonSerializable {
    ...
    implicit val serializeOrientation: Serializable[Orientation] = (orientation: Orientation, depth: Int) => s"""${getTab(depth)}"direction": "${Orientation.toChar(orientation).toString}""""

    implicit val serializeMower: Serializable[Mower] = (mower: Mower, depth: Int) => s"""{\n${serializePosition.serialize(mower.getPosition, depth + 1) + ",\n" + serializeOrientation.serialize(mower.getOrientation, depth + 1) + "\n" + getTab(depth)}}"""

    implicit val serializeInstruction: Serializable[List[Instruction]] = (instructions: List[Instruction], depth: Int) => s"""${getTab(depth)}"instructions": [${instructions.map(instruction => s""""${instruction.toString}"""").mkString(",")}]"""

    implicit val serializeLand: Serializable[LandConfiguration] = (landConfiguration: LandConfiguration, depth: Int) =>
        s"""{
           |"limite": {
    ...
    implicit val serializeMowerHandler: Serializable[MowerHandler] = (mowerHandler: MowerHandler, depth: Int) =>
        s"""${getTab(depth)}{
           |${getTab(depth + 1) + "\"debut\": " + JsonSerializable.serializeMower.serialize(mowerHandler.start, depth + 1) + ",\n" + JsonSerializable.serializeInstruction.serialize(mowerHandler.instructions, depth + 1) + ",\n" + getTab(depth + 1) + "\"fin\": " + JsonSerializable.serializeMower.serialize(mowerHandler.end, depth + 1)}
           |${getTab(depth)}}""".stripMargin
    ...
    implicit val serializeLandResult: Serializable[LandResult] = (landResult: LandResult, depth: Int) =>
        s"""{
           ...
           |${landResult.mowers.map(m => JsonSerializable.serializeMowerHandler.serialize(m, depth + 1)).reduceOption((a, b) => a + ",\n" + b).getOrElse("")}
           |${getTab(depth)}]
           |}""".stripMargin
}
