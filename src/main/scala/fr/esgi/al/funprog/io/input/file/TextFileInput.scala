    ...
    override def getInput(): LandConfiguration = {
        ...
        f.lines.toList match {
            case ::(head, next) => {
                ...
                val mowers = next.grouped(2).map(group => {
                    val mower = group.headOption.getOrElse(throw DonneesIncorectesException("Line is empty"))
                    val instructions = group.lastOption.getOrElse(throw DonneesIncorectesException("Line must be mower instructions"))
